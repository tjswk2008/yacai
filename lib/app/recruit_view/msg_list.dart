...
class MsgListState extends State<MsgList> with SingleTickerProviderStateMixin {
  ...
  Widget buildJobItem(BuildContext context, int index) {
    ...
    return new InkWell(
        ...
        child: new CommunicateListItem(post));
  }

  void getConversationList(int page) async {
    currentPage = page;
    // if(page > totalPage) {
    //   return;
    // }
    ...
    Api().getConversationList(hunterId)
      .then((Response response) {
        ...
        if (response.data['code'] == 1) {
          ...
          //     _posts = CommunicateModel.fromJson(response.data['list']);
          //   } else {
          //     CommunicateModel.fromJson(response.data['list']).forEach((item) {
          //       _posts.add(item);
          //     });
          //   }
          // });
