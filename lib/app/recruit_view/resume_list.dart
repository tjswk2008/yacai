...
class ResumeTabState extends State<ResumeTab> {
                ...
                new DropdownMenuBuilder(
                    builder: (BuildContext context) {
                      return new DropdownTreeMenu(
                        ...
                        itemBuilder: (BuildContext context, dynamic data,
                          ...
                          if (!selected) {
                            return new DecoratedBox(
                                ...
                                child: new Padding(
                                    padding:
                                        EdgeInsets.only(left: 15.0 * factor),
                                    child: new Row(
                                      children: <Widget>[
                                        new Text(data['title'],
                                            style: TextStyle(
                          ...
                          return new SizedBox(
                            ...
                            child: new Padding(
                              ...
                              child: new Text(
                                data,
                                style: new TextStyle(
                                    color: color, fontSize: 24 * factor),
                              ),
                            ),
                          );
                        },
                        ...
                        data: [
                          {'title': '标记状态', 'children': markers},
                          {'title': '邀请函状态', 'children': invitations}
                        ],
                      );
                    },
                    height: 320.0 * factor),
        ...
        child: new Column(
          children: <Widget>[
            ...
            new Expanded(
                child: new Stack(
              children: <Widget>[
                (company.verified == null || company.verified == 0)
                    ? Center(
                        child: Text('您的公司尚未认证，请先前往认证',
                            style: TextStyle(fontSize: 28 * factor)))
                    : _personalInfos.length != 0
                        ? new Padding(
                            padding: EdgeInsets.only(top: 15.0 * factor),
                            child: EasyRefresh(
                                onRefresh: () {
                                  return getResumeList(1);
                                },
                                onLoad: () async {
  ...
  Future<void> getResumeList(page) async {
    currentPage = page;
    if (page > totalPage) {
      return;
    }
    ...
    return Api()
        .getResumeList(prefs.getString('userName'), widget.jobId, timeReq,
            academic, salary, mark, accepted, page, widget.type)
